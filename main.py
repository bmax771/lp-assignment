...
def input_processing() -> list:
    ...
    if choice in ["artifacts", "delete"]:
        ...
        return [choice, str(page_size), str(project_name), str(repo_name)]

    if choice == "project_repos":
        project_name = input("Please enter the project name: ")
        page_size = input("[OPTIONAL] Please enter an integer value of number of items per page (default = 100): ")
        return [choice, str(page_size), str(project_name)]

    if choice == "list_project":
        project_name = input("Please enter the project name: ")
        page_size = input("[OPTIONAL] Please enter an integer value of number of items per page (default = 100): ")
        ...
        return [choice, str(page_size), str(project_name), str(name_of_owner), str(is_public).lower(),
                str(with_details).lower()]
    if choice == "all_repos":
        page_size = input("[OPTIONAL] Please enter an integer value of number of items per page (default = 100): ")
        return [choice, str(page_size)]

    print("NO CHOICE WAS ENTERED.")
    return []


async def main(inputs: list):
    if inputs[0] == "artifacts":
        url_string = (getenv("HARBOR_API_URL") + "/projects/" + input_list[2] + "/repositories/"
                      + input_list[3] + "/artifacts?page=1")
        if inputs[1]:
...
if __name__ == "__main__":
    ...
    if check_if_env_got_loaded:
        input_list = input_processing()
        if len(input_list) >= 1:
            print(run(main, input_list))
    else:
